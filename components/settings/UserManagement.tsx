...
export default function UserManagement({
  ...
  console.log("[UserManagement] users:", users.map((u) => ({ id: u.id, name: u.full_name, avatar_url: u.avatar_url, banner_color: u.banner_color })));
  ...
  return (
    <div className="bg-[#111111] rounded-xl border border-[#1f1f1f]">
      ...
      {users.length === 0 ? (
        ...
        const cardUsers = sorted.filter(
          (u) => u.avatar_url !== null || (u.banner_color !== null && u.banner_color !== "#00f5c4")
        );
        ...
        return (
          <div className="p-4 space-y-4">
            ...
            {/* ── Regular rows ─────────────────────────── */}
            {rowUsers.length > 0 && (
              <div className={`bg-[#111111] rounded-xl border border-[#1f1f1f] divide-y divide-[#1f1f1f] ${cardUsers.length > 0 ? "mt-3" : ""}`}>
                {rowUsers.map((user) => {
                  const isMe = user.id === currentUserId;
                  const isRemoving = removingId === user.id;
